    ...
    <div className="flex flex-col gap-8">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    title={`View ${MEMORY_BACKEND_LABELS[effectiveMemoryBackend(key, globalMemoryBackend)]} memories`}
                  >
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <Input
            label={`Memory Bank ID${profileForm.memoryEnabled ? " (required)" : " (optional)"}`}
          ...
          <label className="text-sm font-medium">
            ...
            <select
              className="mt-1 w-full rounded-lg border border-border bg-surface px-3 py-2"
              value={profileForm.memoryBackend}
              onChange={(event) => setProfileForm((value) => ({ ...value, memoryBackend: event.target.value }))}
              disabled={!profileForm.memoryEnabled}
            >
      ...
      >
        {editingKey && <div className="flex flex-col gap-4">
          ...
          <Input label={`Memory Bank ID${editingKey.memoryEnabled !== false ? " (required)" : " (optional)"}`}
          ...
          <label className="text-sm font-medium">
            ...
            <select
              className="mt-1 w-full rounded-lg border border-border bg-surface px-3 py-2"
              value={editingKey.memoryBackend || ""}
              onChange={(event) => setEditingKey((value) => ({ ...value, memoryBackend: event.target.value }))}
              disabled={editingKey.memoryEnabled === false}
            >
              <option value="">Herda config global/auto ({MEMORY_BACKEND_LABELS[globalMemoryBackend]})</option>
              <option value="neo4j">Neo4j</option>
