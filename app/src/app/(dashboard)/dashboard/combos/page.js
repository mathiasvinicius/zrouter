...
export default function CombosPage() {
  ...
  // Keep independent combo options when the routing strategy returns to fallback.
  const handleSetComboStrategy = async (comboName, patch) => {
    try {
      const updated = { ...comboStrategies };
      const next = { ...(updated[comboName] || {}), ...patch };
      if (next.fallbackStrategy === "fallback") delete next.fallbackStrategy;
      if (!next.thinkingLevel || next.thinkingLevel === "default") delete next.thinkingLevel;
...
const THINKING_OPTIONS = [
  { value: "default", label: "Thinking: client / provider default" },
  ...
  { value: "ultra", label: "Thinking: ultra" },
];
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3 sm:shrink-0">
          ...
          <div className="flex w-full flex-col gap-1.5 sm:w-[230px]">
            ...
            <Select
              ...
              value={strategy.thinkingLevel || "default"}
              onChange={(e) => onSetStrategy({ thinkingLevel: e.target.value })}
              selectClassName="py-1.5 text-xs"
              aria-label={`Thinking level for ${combo.name}`}
              title="Applied to every model in this combo; unsupported levels are adapted to each provider"
            />
          </div>
