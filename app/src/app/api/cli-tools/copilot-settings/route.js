...
const has9RouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "ZRouter" || entry.name === "9Router");
};

const getZRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "ZRouter" || entry.name === "9Router") || null;
};
...
export async function POST(request) {
  try {
    ...
    const idx = config.findIndex((e) => e.name === "ZRouter" || e.name === "9Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "ZRouter" && e.name !== "9Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));
