...
async function runNativeCodexRequest(model, reasoning, comboThinkingLevel) {
  const body = {
    model,
    input: "hello",
    stream: false,
...
describe("native Codex passthrough thinking suffixes", () => {
  ...
  it("lets the combo picker override client effort and model suffix without losing summary", async () => {
    const body = await runNativeCodexRequest("gpt-5.6-sol(ultra)", {
      effort: "high",
      summary: "detailed",
    }, "low");
