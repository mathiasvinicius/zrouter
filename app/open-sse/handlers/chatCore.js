...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, identityContext, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, comboThinkingLevel }) {
  ...
  // An explicit combo selection overrides client/provider intent for each actual
  // member model. The translator clamps or omits levels unsupported upstream.
  const hasComboThinking = COMBO_THINKING_LEVELS.has(comboThinkingLevel);
  const clientReasoningMetadata = hasComboThinking && body.reasoning && typeof body.reasoning === "object"
    ? { ...body.reasoning }
    : null;
  if (hasComboThinking) {
    body = { ...body };
    applyThinking(sourceFormat, stripThinkingSuffix(model), body, provider,
      comboThinkingLevel === "none" ? { mode: "none" } : { mode: "level", level: comboThinkingLevel });
  }
  ...
  if (passthrough) {
    ...
    translatedBody = { ...body, model: stripThinkingSuffix(effectiveUpstreamModel) };
    if (provider === "codex") {
      const suffixThinking = {};
      if (!hasComboThinking) applyThinking(sourceFormat, upstreamModel, suffixThinking, provider);
      const effort = suffixThinking.reasoning_effort || translatedBody.reasoning_effort;
      if (effort) {
        const reasoning = clientReasoningMetadata || translatedBody.reasoning;
