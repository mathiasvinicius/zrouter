---
name: zrouter-video
description: Use para gerar vídeo pelo fluxo assíncrono /v1/videos do ZRouter somente após confirmar uma conexão xAI ativa nesta instalação.
---

# ZRouter — Video Generation (xAI Grok Imagine)

Leia a [skill de entrada](../zrouter/SKILL.md) para endereço e autenticação. Confirme a conexão xAI antes de enviar um trabalho.
...
```bash
curl -X POST "$ZROUTER_URL/v1/videos/generations" \
...
```bash
curl "$ZROUTER_URL/v1/videos/abc123" \
  -H "Authorization: Bearer $ZROUTER_KEY" \
