...
3. **Open Notebook por API.** Usar o serviço existente (`/api/notebooks`, `/api/sources`, `/api/search`) com credencial de serviço server-side. O `POST /api/search` atual pesquisa globalmente e não aceita `notebook_id`: o ZRouter deve filtrar resultados por IDs autorizados **antes de exibir ou injetar** qualquer trecho; para isolamento forte, adicionar filtro server-side ao Open Notebook antes de liberar múltiplos usuários. Retornar passagens e referências, não executar `/api/search/ask` para cada chamada: isso acrescentaria uma segunda geração e custo. Não copiar/alterar dados do SurrealDB. Testar uma busca com fonte real autorizada.
...
## O que vale trazer de outros projetos

| Recurso | Decisão | Motivo |
| --- | --- | --- |
| Fontes Notion via MCP/REST | Sim, leitura primeiro | Responde perguntas com páginas reais; seis operações do OmniRoute são referência, mas escrita exige permissão separada. |
| Busca + trechos citáveis do Open Notebook | Sim, via API | Reutiliza ingestão e indexação que já rodam no Zenith, sem duplicar SurrealDB. |
| Transparência de rota, latência, fallback e custo | Sim | Ajuda a diagnosticar por que uma combinação usou outro modelo, sem criar novas estratégias opacas. |
| Estado de providers e teste de geração por modelo | Sim | Evita anunciar provider configurado mas inutilizável. |
| Importação seletiva de conexões/combinações | Depois | Útil para cutover; deve excluir segredos e preservar IDs/banks de forma explícita. |
| Catálogo completo do OmniRoute, A2A, Electron e múltiplos roteadores embutidos | Não agora | Aumenta manutenção e superfície sem resolver os fluxos centrais. |
| Podcasts/estúdio do Open Notebook dentro do roteador | Não | Já existe no Open Notebook e não pertence ao caminho crítico de roteamento. |
...
- APIs dos primeiros providers candidatos: `https://docs.requesty.ai/api-reference/inference-apis` e `https://novita.ai/docs/guides/llm-reasoning`.
